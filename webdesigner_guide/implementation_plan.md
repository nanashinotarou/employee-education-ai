...
### Phase 8：コンテンツ肉付け（重点3箇所）

**担当**：Cursor  
**方針**：全ページ均等に手を入れない。精査で特定した「明らかに薄い3箇所」だけ補強する。HTMLの構造は既存のまま流用し、テキスト追加が基本。コードは既存スタイル（`.callout`・`icon-list`・テーブルなど）を再利用すること。

---

#### 8-1：page02 — Step 02・Step 04 のアコーディオン補強

**対象ファイル**：`page02_workflow.html`  
**現状**：デザイナーが主役の Step 02・04 が各1行。開いた読者が「それだけ？」となる。Step 05〜08（エンジニア工程）は現状のまま触らない。

**Step 02（サイトマップ / 情報設計）に追加する内容**：

- サイトマップとは「どんなページが何階層で存在するか」を整理した全体地図
- ここで決めること：ページ名・グローバルナビの項目・各ページの目的・親子関係
- クライアントとの最初の合意ポイント。「後でページを追加したい」が起きやすいので、ここでしっかり範囲を固める
- ツール例：FigJam・Miro・Googleスライド（視覚化して共有するのが一般的）

**Step 04（デザインカンプ）に追加する内容**：

- Figma で色・画像・フォントを乗せた「完成イメージ図」を作る工程
- PCとSP（スマホ）の2パターンが基本。レスポンシブの折れ方もここで決める
- ボタン・カード・ヘッダーなどを「コンポーネント化」して使い回す設計も重要
- デザインカンプはコーダーへの指示書。色コード・余白px・フォント名が読み取れる状態に
- クライアントへ提出 → 確認 → 修正、が1〜2ラウンド入るのが通常

---

#### 8-2：page05 — コード生成AIセクションに比較表を追加

**対象ファイル**：`page05_ai.html`  
**現状**：「画像生成AI」「デザイン生成AI」には比較表があるのに「コード生成AI」だけpillリストのみ。統一感が崩れている。

**追加内容**：既存 `ai-category` ブロックの上に、他セクションと同形式の比較表を追加する。

| ツール | 特徴 | 向いている使い方 |
|---|---|---|
| Cursor | AIエディター内蔵。実装作業に特化 | HTML/CSS実装・修正・デバッグ |
| Claude Code | 設計・計画・レビューに強い | 要件整理・仕様書作成・コードレビュー |
| GitHub Copilot | エディター補完型。既存コードへの追記が得意 | コーダーの補佐ツール |
| Bolt.new | ブラウザ完結でWebアプリを生成 | プロトタイプ・アイデア検証 |

表の下に補足文を1段落：「このサイト自体も Claude Code → Cursor → Antigravity の3ツール分業で作られている。"コードを書く"のではなく"指示する"立場でも十分使いこなせる。」

---

#### 8-3：page03 — UX/UIセクションに実践視点を1ブロック追加

**対象ファイル**：`page03_design.html`  
**現状**：定義カード2枚のみ。「で、何をすればいいの？」で終わる。

**追加内容**：既存2カードの下に `.callout` スタイルで「UXチェックの3問」を追加。

```
デザインを見直す前に、この3つを問いかけてみよう：
① このページで訪問者は「何をしたいか」を言語化できているか？
② 次のアクション（ボタン・リンク）が迷わず目に入るか？
③ 迷ったときに「戻れる」「助けを求められる」設計があるか？

UXはセンスではなく、問いの精度で決まる。
```
...
- [x] `page02_workflow.html`：Step 02 の `<div class="body">` が4〜5行の内容になっている
- [x] `page02_workflow.html`：Step 04 の `<div class="body">` が4〜5行の内容になっている
- [x] `page05_ai.html`：コード生成AIセクションに4ツールの比較表が追加されている
- [x] `page03_design.html`：UX/UIセクション末尾に「3問チェック」のコラムが追加されている

---

---

### Phase 9：受講生フレンドリー化（温度・入口・著者の顔）

**担当**：Cursor  
**方針**：受講生（Canva勢・完全初心者含む）が読んだとき、「どこから読めば？」「困ったら？」「誰が作ったの？」への答えを補完する。コンテンツ追加5箇所。HTMLコードの詳細はCursorが判断してよい。スタイルは既存CSS変数を流用すること。

---

#### 9-1：index.html — 推奨読み順ブロック

**対象ファイル**：`index.html`  
**挿入場所**：`<section class="card" id="learn">` 内、`<p class="section-lead">` の直下・`.nav-cards` グリッドの直前

**追加内容**：
- コンパクトなinfo帯（1行）で「はじめての方へ → Page 01 → Page 05 → Page 07 の順がおすすめ」と案内
- アイコン：`fa-route` など経路系のFont Awesomeアイコン
- スタイル：`--accent-2`（緑）の左ボーダーか薄い背景帯。既存の `.callout` より小さく、コンパクトに収める
- テキスト例：「**はじめての方へ：** Page 01（仕事を知る）→ Page 05（AIと活用）→ Page 07（始め方）の順で読むと全体像が掴みやすいです」

---

#### 9-2：全8ページ — フッターへの問い合わせ一言追加

**対象ファイル**：`index.html` / `page01_what.html` ～ `page07_start.html`（計8ファイル）  
**挿入場所**：各ページ `<footer>` 内、`© 2026 H.K` の `<p>` の直後

**追加内容**（全ページ共通、1行のみ）：
```
<p style="margin-top:6px;">わからないことがあれば、H.K. に直接声をかけてください。</p>
```
- スタイル：フッターの既存 `rgba(255,255,255,0.6)` で問題なし。特別な装飾は不要
- 意図：社内研修教材として「質問してよい」という安心感を与える

---

#### 9-3：page07 — 学習ロードマップ Step 1 に Canva を追加

**対象ファイル**：`page07_start.html`  
**対象箇所**：`.roadmap-step[data-step="1"]`

**修正前**：
```
<h5>Figma でトレース模写（3 作品）</h5>
<p>好きなサイトを観察し、同じものを作る練習。デザイン眼と操作に慣れる。</p>
```

**修正後（内容方向性）**：
- `<h5>` を「Canva または Figma でトレース模写（3 作品）」に変更（Canvaを先に書く）
- `<p>` を「好きなサイトを観察し、同じものを作る練習。Canva ユーザーはそのままCanvaで始めてOK。デザイン眼と操作に慣れることが目的で、ツールの種類は問いません。」程度の内容に補強

---

#### 9-4：index.html — 著者紹介ブロック（プレースホルダー）

**対象ファイル**：`index.html`  
**挿入場所**：`<section class="card" id="about">` の3カラムグリッド `<div>` の閉じタグ直後、`<style>` タグの前

**追加内容**：著者（H.K.）の一言ブロックを追加する。

- スタイル：左ボーダー（`border-left: 4px solid var(--accent)`）＋薄い背景（`#fdf6f0`）。Font Awesomeアイコンは**使わない**
- margin-top: 20px 程度で3カラムグリッドとの間隔を取る
- 以下はプレースホルダーテキスト。**H.K. に確認の上、差し替えてもらう**：

```
確定テキスト：
みなさんお疲れ様です。制作(したAIを監督しました)H.K.です。
動画を観て、課題をこなして、また次のテーマへ——その繰り返しの中で、「普段の研修まとめ教材だけでなく、もう少し踏み込んだ地図があれば」と思って今回の教材を作りました。
半年後・一年後に、今より一歩踏み込んだ人材でいるために。読んで損はないと思います。
― H.K.
```

- ブロックの末尾の `― H.K.` は上記テキストに含まれているため、署名の二重表記に注意すること

---

#### 9-5：page01_what.html — SKILL 04 付近にH.K.の一人称コメント

**対象ファイル**：`page01_what.html`  
**挿入場所**：`#skills` セクション内、「大切なこと」の `.point-card` の直後

**追加内容**：H.K.の実体験として「このサイト自体をAIだけで作った」ことへの言及。

- スタイル：9-4 と同じ（左ボーダー `var(--accent)` ＋薄い背景 `#fdf6f0`、アイコンなし）
- class名を `.author-note` で統一し、CSSを `<style>` に追加：
  ```css
  .author-note {
  ...
  ```
- index.html の 9-4 ブロックにも同じ `.author-note` クラスを適用してよい（スタイル定義はそれぞれのページに記述）
- 以下はプレースホルダーテキスト。**H.K. に確認の上、差し替えてもらう**：

```
確定テキスト：
正直に言うと、私はコードを一行も書いていません。書けないし、書くつもりもなかった。このサイトを作るために用意したのは、「こういうものを作りたい」という意図だけです。それをAIに伝え続けた結果が、いまあなたが読んでいるこのページです。アイデアを言葉にできる人なら、誰でもできます。
― H.K.
```
...
- [x] `index.html`：`#learn` セクションの nav-cards 直前に推奨読み順の帯（1行callout）が追加されている
- [x] `index.html`：フッターに「わからないことがあれば、H.K. に直接声をかけてください。」が追加されている
- [x] `page01_what.html`：フッターに同上の一言が追加されている
- [x] `page02_workflow.html`：フッターに同上の一言が追加されている
- [x] `page03_design.html`：フッターに同上の一言が追加されている
- [x] `page04_tools.html`：フッターに同上の一言が追加されている
- [x] `page05_ai.html`：フッターに同上の一言が追加されている
- [x] `page06_client.html`：フッターに同上の一言が追加されている
- [x] `page07_start.html`：フッターに同上の一言が追加されている
- [x] `page07_start.html`：Roadmap Step 1 の `<h5>` が「Canva または Figma でトレース模写（3 作品）」になっている
- [x] `page07_start.html`：Roadmap Step 1 の `<p>` に Canva ユーザー向けの補足が追記されている
- [x] `index.html`：`#about` セクションの3カラム下に `.author-note` 著者ブロック（プレースホルダー付き）が追加されている
- [x] `page01_what.html`：`#skills` の `.point-card`（大切なこと）直後に `.author-note` 著者コメント（プレースホルダー付き）が追加されている
